...
- **`max_model_len`**:不做硬性拒绝(vLLM 启动时自己会检查「KV 池至少装得下一条
  max_model_len 序列」,装不下直接拒绝启动 —— 失败模式是"起不来",不是"悄悄跑错"),
  但**返回一个估算提示**:按当前 vram_budget / util 与该模型已知的 KV 单价估 KV 容量,
  给出「预计并发 N.NNx」。已知单价示例:qwen3.8 系列 65.3 KiB/token(2026-09-21 实测)。
